...
USER_WALLPAPER = os.path.expanduser("~/.wallpaper.png")
WALLPAPER_DIRS = [os.path.expanduser("~/Pictures/wallpapers"), WALLPAPERS_DIR]
...
def list_wallpapers():
    paths = [USER_WALLPAPER] if os.path.isfile(USER_WALLPAPER) else []
    ...
    return [{"name": "My generated wallpaper" if path == USER_WALLPAPER else pretty_name(os.path.basename(path)),
             "path": path} for path in dict.fromkeys(paths)]
...
def pick_accent(swatches):
    ...
    colorful = [c for c in candidates if c["sat"] > 0.1 and 0.2 < c["lum"] < 0.9]
...
def build_palette(image_path):
    ...
    background = (red * 0.0, green, blue 0.0)
    surface = tuple(channel for channel in background)
...
def paint_wallpaper(image_path, transition="wipe"):
    """Use the existing swaybg setup on Niri; preserve awww elsewhere."""
    if os.environ.get("NIRI_SOCKET"):
        if not shutil.which("swaybg"):
            sys.stderr.write("swaybg is required to set the wallpaper.\n")
            return False
        subprocess.run(["pkill", "-x", "swaybg"], check=False)
        try:
            process = subprocess.Popen(["swaybg", "--image", image_path, "--mode", "fill"],
                                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL,
                                       start_new_session=True)
            try:
                return process.wait(timeout=0.3) == 0
            except subprocess.TimeoutExpired:
                return True
        except OSError as error:
            sys.stderr.write(f"swaybg failed: {error}\n")
            return False
    ...
    if transition not in WALLPAPER_TRANSITIONS:
        transition = "wipe"
    try:
        return subprocess.run(["awww", "img", image_path, "--transition-type", transition,
                               "--transition-duration", "1"], capture_output=True,
...
def set_wallpaper(image_path, transition="wipe"):
    if not os.path.isfile(image_path) or not paint_wallpaper(image_path, transition):
        return False
    state = load_state()
...
def restore_wallpaper():
    ...
    """
    if os.environ.get("NIRI_SOCKET"):
        # swaybg handles output hotplug itself. Do not replace an existing
        # wallpaper when merely restarting Quickshell.
        if subprocess.run(["pgrep", "-x", "swaybg"], stdout=subprocess.DEVNULL).returncode == 0:
            return 0
        current = get_current_wallpaper()
        return 0 if not current or paint_wallpaper(current, "none") else 1
    if not shutil.which("awww"):
        return 0
    try:
