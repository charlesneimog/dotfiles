...
def lava(paint):
    ...
    metal = {
        "shine": mix(muted, WHITE, 0.5),
        "light": muted,
        "mid": mix(muted, ground, 0.42),
        "dark": mix(muted, ground, 0.66),
    ...
    for index in range(LAVA_FRAMES):
        ...
        for key, radius, phase, cycles, sway, sway_phase in LAVA_BLOBS:
            ...
            blobs.append(
                (paint[key], radius, x, y, 1 / (1 + 0.14 * speed), 1 + 0.32 * speed)
            )
...
GHOST_HEM = (["obbobbbobbbobo", "oo.oo.ooo.oo.o"], ["obobbbobbbobbo", "o.oo.ooo.oo.oo"])
...
def critter_key(paint, key):
    body = paint[key]
    return {
        "o": mix(body, BLACK, 0.62),
        "b": body,
        "l": mix(body, WHITE, 0.4),
        "d": mix(body, BLACK, 0.22),
        "e": mix(paint["ground"], BLACK, 0.6),
        "w": WHITE,
        "n": mix(paint["red"], WHITE, 0.35),
...
def pond_path(kind):
    ...
    for step in range(480):
        ...
        if kind == 0:
            ...
            points.append(
                (POND_X + 3 + 9 * math.cos(-a), POND_Y - 4 + 11 * math.sin(-a))
            )
        else:
            points.append(
                (POND_X + 14 * math.sin(a), POND_Y + 19 * math.sin(a) * math.cos(a) + 2)
            )
    lengths = [0.0]
...
def koi(paint):
    ...
    deep, middle, shallow = (
        mix(ground, blue, 0.24),
        mix(ground, blue, 0.32),
        mix(ground, blue, 0.40),
    )
    stone_tones = (
        paint["muted"],
        mix(paint["muted"], ground, 0.3),
    ...
    for n in range(count):
        ...
        seeds.append(
            (
                POND_X + POND_RX * 1.08 * math.cos(a),
                POND_Y + POND_RY * 1.08 * math.sin(a),
                stone_tones[(n * 7) % 3],
            )
        )
    ...
    for y in range(HEIGHT):
        for x in range(WIDTH):
            ...
            if e <= 1:
                ...
                ranked = sorted(
                    ((x + 0.5 - sx) ** 2 + (y + 0.5 - sy) ** 2, tone, sx, sy)
    ...
    fish = (
        (pond_path(0), 1, 0.00, paint["text"], paint["red"], {1, 2, 3, 7, 8}),
        (
            pond_path(1),
            1,
            0.35,
            paint["yellow"],
            mix(paint["yellow"], WHITE, 0.5),
            {0, 1},
        ),
    ...
    for index in range(KOI_FRAMES):
        ...
        for path, laps, offset, body, patch, patches in fish:
            ...
            for j, (x, y) in enumerate(spine):
                ...
                wiggled.append(
                    (x + nx * sway, y + ny * sway, nx, ny, tx / norm, ty / norm)
                )
        ...
        for wiggled, body, patch, patches in bodies:
            ...
            for side in (1, -1):
                frame.disc(
                    x + nx * side * 2.3 - tx * 0.6,
                    y + ny * side * 2.3 - ty * 0.6,
                    1.0,
                    fin,
                )
            ...
            for side in (1, -1):
                frame.disc(
                    x - tx * 1.1 + nx * side * 1.0,
                    y - ty * 1.1 + ny * side * 1.0,
                    1.1,
                    fin,
                )
        ...
        for start, rx, ry in ripples:
            ...
            for y in range(int(ry - radius - 2), int(ry + radius + 3)):
                for x in range(int(rx - radius - 2), int(rx + radius + 3)):
                    if (
                        abs(math.hypot(x + 0.5 - rx, y + 0.5 - ry) - radius) < 0.5
                        and (x, y) in water
                    ):
        ...
        for px, py, radius, notch in pads:
            for y in range(int(py - radius - 1), int(py + radius + 2)):
                for x in range(int(px - radius - 1), int(px + radius + 2)):
                    ...
                    angle = (math.atan2(dy, dx) - notch + math.pi) % (
                        2 * math.pi
                    ) - math.pi
...
INVADER_ROWS = (
    ("accent", SQUID, 10),
    ("green", CRAB_INVADER, 19),
    ("yellow", CRAB_INVADER, 28),
    ("blue", OCTOPUS, 37),
)
INVADER_FRAMES = 80
INVADER_DELAY = 8
...
def write(scene, paint, path, magick, current=lambda: True):
    ...
    try:
        subprocess.run(
            command, input=stream, capture_output=True, timeout=60, check=True
        )
        if not current():
            os.remove(staging)
            return False
...
def push(paint):
    ...
    try:
        with open(LOG_FILE, "w", encoding="utf-8") as log:
            subprocess.Popen(
                [sys.executable, os.path.abspath(__file__), "draw"],
...
USAGE = (
    "greeting.py draw | choose <" + "|".join(CHOICES) + "> | seed"
    " | <" + "|".join(SCENES) + "> <out.gif>"
)
