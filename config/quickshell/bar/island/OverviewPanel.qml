...
ColumnLayout {
    ...
    readonly property int columns: Math.max(1, Math.min(5, NiriService.workspaces.length))
    readonly property int rows: Math.ceil(NiriService.workspaces.length / root.columns)
    readonly property int gap: 10

    // The monitor the cells model. Positions are scaled from its real size
    // rather than an assumed aspect ratio.
    readonly property var monitor: NiriService.monitors[0] ?? null
    ...
    function selectBy(delta: int): void {
        const ids = NiriService.visibleIds
        if (!ids.length) return
        const index = Math.max(0, ids.indexOf(root.selectedId))
        root.selectedId = ids[(index + delta + ids.length) % ids.length]
    }

    function activate(): void {
        NiriService.focus(root.selectedId)
        root.closed()
    }
    ...
    Component.onCompleted: {
        NiriService.loadClients()
        NiriService.loadMonitors()
        void ToplevelManager.toplevels
        root.selectedId = NiriService.activeId
        ...
        GridLayout {
            ...
            Repeater {
                ...
                Item {
                    ...
                    readonly property int workspaceId: cell.modelData.id
                    readonly property bool focused: NiriService.activeId === cell.workspaceId
                    readonly property bool selected: root.selectedId === cell.workspaceId
                    readonly property var windows: NiriService.clientsOn(cell.workspaceId)
                                ...
                                MouseArea {
                                    ...
                                    onClicked: mouse => {
                                        if (mouse.button === Qt.RightButton) {
                                            NiriService.closeWindow(thumb.modelData.address)
                                            return
                                        }
                                        if (mouse.button === Qt.MiddleButton) {
                                            NiriService.toggleFloating(thumb.modelData.address)
                                            return
                                        }
                                        NiriService.focusWindow(thumb.modelData.address)
                                        root.closed()
                                    }
                                }
                            }
                    ...
                    Text {
                        anchors.centerIn: parent
                        text: cell.modelData.name || String(cell.modelData.idx)
                    ...
                    MouseArea {
                        ...
                        onClicked: {
                            NiriService.focus(cell.workspaceId)
                            root.closed()
                        }
                    }
                    ...
                    DropArea {
                        ...
                        onDropped: dropped => {
                            ...
                            if (client.workspace.id !== cell.workspaceId) {
                                NiriService.moveClient(client.address, cell.workspaceId)
                                dropped.accept()
                                return
                            }
                            ...
                            if (client.floating) {
                                NiriService.moveFloating(client.address,
                            ...
                            if (under && !under.floating)
                                NiriService.focusWindow(under.address)
