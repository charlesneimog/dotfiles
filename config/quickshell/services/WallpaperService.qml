...
QtObject {
    ...
    readonly property Process applyProcess: Process {
        onExited: exitCode => {
            ...
            } else console.warn("Could not apply wallpaper:", root.pendingWallpaper)
        }
    }
    ...
    readonly property Connections hotplug: Connections {
        target: Quickshell
        function onScreensChanged(): void { root.arriving.restart() }
    }
    ...
    function apply(path: string): void {
        if (!path || root.applyProcess.running)
            return
