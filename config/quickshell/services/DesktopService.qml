...
Singleton {
    ...
    function normalise(list: var): var {
        ...
        for (let index = 0; index < length; index++) {
            ...
            if (!kept || !kept.id || ["pet", "games", "notes", "tasks", "board"].includes(kept.id) || kept.edge)
                continue
    ...
    readonly property var squares: root.widgets
    ...
    readonly property Connections compositor: Connections {
        target: NiriService
        function onActiveIdChanged(): void { if (root.editing) root.edit(false) }
        function onWindowsChanged(): void { if (root.editing) root.edit(false) }
    }
}
