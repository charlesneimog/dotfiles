...
Singleton {
    ...
    readonly property var table: {
        ...
        for (const bind of NiriService.binds) {
            if (!(bind.key || "").startsWith("switch:"))
                add(bind.description ?? "", NiriService.spell(bind))
        }
    ...
    readonly property Process reloader: Process {
        ...
        onExited: NiriService.loadBinds()
    }
    ...
    readonly property Timer settle: Timer {
        interval: 400
        onTriggered: NiriService.loadBinds()
    }
}
