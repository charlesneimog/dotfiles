...
//
// The radio follows NetworkManager over D-Bus, including external changes.
Singleton {
    id: root
    ...
    readonly property NetworkDevice wifiDevice:
        root.devices.find(device => device.type === DeviceType.Wifi && device.connected)
            ?? root.devices.find(device => device.type === DeviceType.Wifi) ?? null
    readonly property NetworkDevice wiredDevice:
        root.devices.find(device => device.type === DeviceType.Ethernet && device.connected)
    ...
    readonly property bool busy: root.connectProcess.running
    ...
    readonly property Process connectProcess: Process {
        ...
        onExited: exitCode => {
            if (exitCode !== 0 && !root.error)
                root.error = "Connection failed. Check the password and try again."
            root.busySsid = ""
            root.refresh()
            root.scan(false)
        }
    ...
    function scan(rescan = false): void {
        ...
        if (root.listProcess.running) return
        root.scanning = true
