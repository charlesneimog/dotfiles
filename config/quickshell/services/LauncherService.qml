...
QtObject {
    ...
    // Quickshell watches desktop entries, including Flatpak XDG exports.
    readonly property var applications: DesktopEntries.applications.values
        .filter(entry => !entry.noDisplay)
        .map(entry => ({ kind: "app", id: entry.id.endsWith(".desktop") ? entry.id : entry.id + ".desktop", name: entry.name,
            subtitle: entry.comment || entry.genericName || "Application",
            keywords: [entry.name, entry.genericName, ...entry.keywords].join(" ").toLowerCase(),
            icon: entry.icon, exec: entry.execString, wmclass: entry.startupClass }))
    ...
    function refresh(): void {
        // DesktopEntries updates automatically when applications change.
    }
    ...
    function windows(term: string): var {
        const found = []
        for (const client of NiriService.clients) {
    ...
    function launchDesktop(desktopId: string): void {
        ...
        else Quickshell.execDetached(["gtk-launch", desktopId])
    }
}
