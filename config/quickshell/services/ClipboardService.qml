...
Singleton {
    ...
    readonly property Process watcher: Process {
        onExited: {
            if (root.ready && root.wanted.length > 0)
                root.retry.restart()
        }
        stderr: StdioCollector {
            onStreamFinished: {
                if (root.ready && text.trim() !== "")
                    console.warn("Clipboard watcher:", text.trim())
            }
        }
    }

    // Defer replacement until the old process has exited. A failed watcher
    // retries slowly instead of being permanently disabled after a reload.
    readonly property Timer retry: Timer {
        interval: 5000
        onTriggered: root.tend()
    }
    function tend(): void {
        if (!root.ready || root.wanted.length === 0 || root.watcher.running) return
        root.watcher.command = root.wanted
        root.watcher.running = true
    }
    Component.onCompleted: { root.ready = true; root.tend() }
